  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 space-y-4">
      ...
      {googleAccounts.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Google Drive</p>
          <div className="space-y-1">
            {googleAccounts.map((a) => (
              <AccountRow key={a.id} label={a.displayName ?? a.email} sublabel={a.email}
                selected={selectedId === a.id} onSelect={() => onSelect(a.id)} onRemove={() => onDisconnectServer(a.id)} />
            ))}
          </div>
        </div>
      )}
      ...
      {filenAccounts.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Filen</p>
          <div className="space-y-1">
            {filenAccounts.map((a) => (
              <AccountRow key={a.id} label={a.displayName ?? a.email} sublabel={a.email}
                selected={selectedId === a.id} onSelect={() => onSelect(a.id)} onRemove={() => onDisconnectServer(a.id)} />
      ...
      {fsAccounts.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Local file system</p>
          <div className="space-y-1">
            {fsAccounts.map((a) => (
              <AccountRow key={a.id} label={a.name}
                selected={selectedId === a.id} onSelect={() => onSelect(a.id)} onRemove={() => onDisconnectFs(a.id)} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 pt-1">
        ...
        <button onClick={onConnectFs}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 active:bg-gray-300 dark:active:bg-gray-500 transition-colors">
          <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
...
function AccountRow({ label, sublabel, selected, onSelect, onRemove }: {
  label: string; sublabel?: string; selected: boolean; onSelect: () => void; onRemove: () => void;
}) {
  return (
    <div onClick={onSelect}
      ...
      <div className="min-w-0 flex-1">
        ...
        {sublabel && <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{sublabel}</p>}
      </div>

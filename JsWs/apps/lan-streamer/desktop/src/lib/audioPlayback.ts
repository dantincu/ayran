...
export class AudioPlayback {
  ...
  enqueueFrame(data: ArrayBuffer): void {
    ...
    // was still crashing from a full cushion to empty almost instantly. If
    // main-thread work between "WS message received" and "data actually
    // handed to the worklet" is itself periodically delayed (e.g. by
    // per-frame allocation/GC, same root cause as the original host-side
    // stalls, just here on this device's main thread), that gap would show
    // up here even though the network-level gap is clean.

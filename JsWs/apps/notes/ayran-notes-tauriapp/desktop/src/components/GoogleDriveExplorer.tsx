...
export default function GoogleDriveExplorer({ account, onDisconnect, onOpenFile, onOpenNotebook }: Props) {
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-3 p-1">
              {files.map(item => (
                <button key={item.itemId}
                  onClick={() => item.isDir ? openFolder(item) : onOpenFile(item, [...breadcrumbs.map(b => b.name), item.name].join(' / '))}
              ...
              {files.map((item, idx) => {
                ...
                return (
                  <div key={item.itemId}
                    ...
                    onClick={(e) => { if ((e.target as HTMLElement).closest('button,input')) return; item.isDir ? openFolder(item) : onOpenFile(item, [...breadcrumbs.map(b => b.name), item.name].join(' / ')); }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <button onClick={() => item.isDir ? openFolder(item) : onOpenFile(item, [...breadcrumbs.map(b => b.name), item.name].join(' / '))}

...
**The indexes** follow the intervals of the folder pairs strategy, read from the config file (`config/folder-pairs-and-notes.json`): a new note item is numbered from `999` **downward, after the largest** — so the first note is `999` and the next `998`. Notes numbered before this (`001`, `002`…) are *outside every interval*: they are read and shown like the others and can be **converted** into an interval. The Notes app edits indexes **all at once** in one dialog — typed over, reordered by dragging or with arrows (the same indexes handed out again in the new order), **normalized** (each interval closes its gaps from its own start, the order kept) or **converted** from one interval to another (only the notes selected for it; whole intervals can be selected with one press) — and writes them in one go in the two phases the section "About updating the note indexes" describes (temporary prefix `t_`, then the final names). Deleting or moving notes offers to normalize what remains. **Moving and copying notes** under another parent gives them, by default, the next available indexes of the new parent; the person can choose to fill its gaps or type indexes of their own.

**Opening a note from a link** inside a web page works: a note's address (`/Book/999?note`) is a link that is put to the person like any page's (*In this tab / In a new tab*) and shows the note's markdown.

**Not built yet:** note sections themselves and the notebook's reserved pair of folders (`03-[note-book]`).

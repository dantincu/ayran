...
class FilenRepository(
    ...
    private fun decryptMasterKeys(
        encrypted: String,
        masterKeyHex: String,
        loginDecryptKey: String,
        v1MasterKey: String = ""
    ): List<String> {
        ...
        val candidates = listOfNotNull(
            loginDecryptKey,
            masterKeyHex,
            v1MasterKey.takeIf { it.isNotEmpty() }
        )
            ...
            try {
                ...
                return try {
                    ...
                    when {
                        ...
                        parsed is JsonObject -> parsed["keys"]?.jsonArray?.map { it.jsonPrimitive.content }
                            ?: listOf(masterKeyHex)
    ...
    fun writeJsonLogFile(
        ...
        api.uploadFileChunk(apiKey, newUuid, 0, parentUuid, uploadKey, encrypted)
        api.markUploadDone(
